...
#include "UnityCG.cginc"
	...
	fixed4 frag(v2f i) : COLOR
	{
		...
		float glowStrength = _Outline * (_SinTime.a);

		fixed alpha_up = tex2D(_MainTex, i.uv + fixed2(0, _MainTex_TexelSize.y * glowStrength)).a;
		fixed alpha_down = tex2D(_MainTex, i.uv - fixed2(0, _MainTex_TexelSize.y * glowStrength)).a;
		fixed alpha_right = tex2D(_MainTex, i.uv + fixed2(_MainTex_TexelSize.x * glowStrength, 0)).a;
		fixed alpha_left = tex2D(_MainTex, i.uv - fixed2(_MainTex_TexelSize.x * glowStrength, 0)).a;
